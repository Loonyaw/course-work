  ...
  if (operationContent) {
    ...
    if (clickedTab) {
      var content = document.querySelector(
        ".operations__content--".concat(clickedTab)
      );
      if (content) {
...
nav.addEventListener("mouseover", function (e) {
  return handleMenuHover("0.5", e);
});
nav.addEventListener("mouseout", function (e) {
  return handleMenuHover("1", e);
});
...
var slider = function () {
  ...
  var activeDot = function (slide) {
    ...
    var dotSlide = document.querySelector(".dots__dot");
    if (dotSlide) {
      var slideData = '[data-slide="'.concat(slide, '"]');
      if (slideData) {
        var dataSlide = document.querySelector(
          '.dots__dot[data-slide="'.concat(slide, '"]')
        );
        if (dataSlide) {
  ...
  var goToSlide = function (slide) {
    slides.forEach(function (s, index) {
      return (s.style.transform = "translateX(".concat(
        100 * (index - slide),
        "%)"
      ));
    });
